...
const PagosScreen = ({navigation}) => {
  // Si estás usando `react-navigation`, añadimos el prop `navigation`
  const data = [
    {
      id: '1',
      name: 'Juan Pérez',
      image: require('../assets/personal.png'),
      message: 'Realizó pago',
    },
  ...
  const renderItem = ({item}) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('DetallesPago', {name: item.name})}>
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity
          ...
          {/* Si estás usando react-navigation */}
          <Image
